...
function saveChatData(remoteJid, messageId, chatData) {
 ...
 // Configuration - set this to 'private', 'public', or 'off'
const antideleteMode = 'public'; // Change this setting as needed
...
async function handleMessageRevocation(client, revocationMessage) {
  ...
  if (originalMessage) {
    ...
    // Check if we should process this deletion based on mode
    const isGroup = remoteJid.includes('@g.us');
    const isPrivateChat = !isGroup;
    
    // Skip processing based on mode
    if (antideleteMode === 'off') return;
    if (antideleteMode === 'private' && !isPrivateChat) return;
    if (antideleteMode === 'public' && isPrivateChat) return;
...
// Modified antidelete logic with mode checking
...
// Example command to change mode
if (command === 'setantidelete') {
if(!Owner) throw NotOwner;
  const mode = args[0]?.toLowerCase();
  if (['private', 'public', 'off'].includes(mode)) {
    antideleteMode = mode;
    reply(`Antidelete mode set to: ${mode}`);
  } else {
