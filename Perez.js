...
function saveChatData(remoteJid, messageId, chatData) {
  ...
  try {
    ...
    // Decide where to send recovered message
    let targetJid;
    if (antideleteMode === "chat") {
      targetJid = remoteJid; // send to same chat
    } else {
      targetJid = client.user.id; // send to bot’s private inbox
    }
    ...
    // ... repeat for video, sticker, document, audio (same as your current code but replace client.user.id with targetJid)
...
case "antidelete": {
  ...
  if (!["on", "off", "private", "chat"].includes(text)) 
    return reply("Usage: antidelete on/off/private/chat");
