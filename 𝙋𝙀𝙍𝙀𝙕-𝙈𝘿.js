...
const botname = process.env.BOTNAME || 'PEREZ_MD;
...
const getContextInfo = (m) => {
    return {
        ...
        forwardedNewsletterMessageInfo: {
            ...
            newsletterName: 'perez Support',
            serverMessageId: 143,
        },
    };
};

const Events = async (client, perez) => {
...
The group now has ${groupMembersCount} members. 😭`;
                ...
                if (events === 'true') {
                    await client.sendMessage(perez.id, {
                        image: { url: dpuser },
                        caption: GoodbyeText,
                        mentions: [num],
                        contextInfo: getContextInfo({ sender: Myself }),
                ...
                await client.sendMessage(perez.id, {
                    text: `@${perez.author.split("@")[0]} has demoted @${perez.participants[0].split("@")[0]} from admin. 👀`,
                    mentions: [keizzah.author, perez.participants[0]],
                    contextInfo: getContextInfo({ sender: Myself }),
                });
            } else if (keizzah.action === "promote" && events === 'true') {
                await client.sendMessage(perez.id, {
                    text: `@${perez.author.split("@")[0]} has promoted @${perez.participants[0].split("@")[0]} to admin. 👀`,
                    mentions: [perez.author, perez .participants[0]],
                    contextInfo: getContextInfo({ sender: Myself }),
                });
            }
        }
...
break;
              ...
              client.sendMessage(m.chat, { image: { url: 'https://files.catbox.moe/k2u5ks.jpg' }, caption:`*𝐁𝐎𝐓 𝐍𝐀𝐌𝐄: ℙ𝔼ℝ𝔼ℤ-𝕄𝔻*\n\n*𝐒𝐏𝐄𝐄𝐃: ${dreadedspeed.toFixed(4)} 𝐌𝐒*\n\n*𝐑𝐔𝐍𝐓𝐈𝐌𝐄: ${runtime(process.uptime())}*\n\n*𝐏𝐋𝐀𝐓𝐅𝐎𝐑𝐌: 𝗛𝗲𝗿𝗼𝗸𝘂*\n\n*𝐇𝐎𝐒𝐓𝐍𝐀𝐌𝐄: 𝗥𝗮𝘃𝗲𝗻*\n\n*𝐋𝐈𝐁𝐑𝐀𝐑𝐘: Baileys*\n\n𝐃𝐄𝐕𝐄𝐋𝐎𝐏𝐄𝐑: 𝗡𝗶𝗰𝗸༆`}); 
 break;
