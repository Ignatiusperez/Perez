...
const botname = process.env.BOTNAME || '𝑵𝑬𝑿𝑼𝑺 𝑴𝑫';
...
const packname = process.env.STICKER_PACKNAME || '𝗡𝗘𝗫𝗨𝗦-𝗠𝗗';
...
function handleIncomingMessage(message) {
  ...
  // Configuration
const ANTIDELETE_MODE = "chat"; // "off", "private", "chat"
const BOT_OWNERS =[''] // Add bot owner numbers
const ALLOWED_GROUPS = ['']// Groups where antidelete should work


async function handleMessageRevocation(client, revocationMessage) {
  // Check if antidelete is enabled
  if (ANTIDELETE_MODE === "off") return;

  const remoteJid = revocationMessage.key.remoteJid;
  const messageId = revocationMessage.message.protocolMessage.key.id;

  // Mode-specific checks
  if (ANTIDELETE_MODE === "private") {
    // Only work in bot owner's chats
    const isOwnerChat = BOT_OWNERS.some(owner => 
      remoteJid === owner || remoteJid === client.user.id
    );
  ...
  if (originalMessage) {
    ...
    let notificationText = `⫸𝗡𝗘𝗫𝗨𝗦 𝗔𝗡𝗧𝗜𝗗𝗘𝗟𝗘𝗧𝗘 𝗥𝗘𝗣𝗢𝗥𝗧⫸\n\n` +
      ...
      `💬 𝗖𝗵𝗮𝘁: ${remoteJid.includes('@g.us') ? 'Group' : 'Private'}\n\n`;

    try {
      // Determine where to send notification based on mode
      let notificationTarget;
      if (ANTIDELETE_MODE === "private") {
        // Send to all bot owners
        notificationTarget = BOT_OWNERS;
      } else {
        // Send to current chat or bot owner based on context
        notificationTarget = [remoteJid];
      }
      ...
      if (originalMessage.message?.conversation) {
        ...
        for (const target of notificationTarget) {
          await client.sendMessage(target, { text: notificationText });
        }
      } 
      else if (originalMessage.message?.extendedTextMessage) {
        const messageText = originalMessage.message.extendedTextMessage.text;
        notificationText += `📝 𝗗𝗲𝗹𝗲𝘁𝗲𝗱 𝗤𝘂𝗼𝘁𝗲𝗱 𝗠𝗲𝘀𝘀𝗮𝗴𝗲: ${messageText}`;
        ...
        for (const target of notificationTarget) {
          ...
          await client.sendMessage(target, { 
            ...
            contextInfo: {
              externalAdReply: {
                title: "DELETED STICKER RECOVERED",
...
// Enhanced message handler with mode support
async function handleIncomingMessageEnhanced(client, message) {
  const remoteJid = message.key.remoteJid;
  const messageId = message.key.id;
  ...
  // Enhanced antidelete handling with modes
  if (ANTIDELETE_MODE !== "off") {
...
// Command to change antidelete mode
async function setAntideleteMode(client, message, args) {
  const validModes = ["off", "private", "chat"];
  const requestedMode = args[0]?.toLowerCase();
  
  if (!validModes.includes(requestedMode)) {
    return await client.sendMessage(message.chat, {
      text: `❌ Invalid mode. Use: ${validModes.join(", ")}`
    });
  }

  ANTIDELETE_MODE = requestedMode;
  
  await client.sendMessage(message.chat, {
    text: `✅ Antidelete mode set to: ${requestedMode.toUpperCase()}\n\n` +
          `• OFF: Disabled\n` +
          `• PRIVATE: Only notifies bot owners\n` +
          `• CHAT: Notifies in the chat where deletion occurred`
  });
}

// Utility function to check if user is bot owner
function isBotOwner(sender) {
  return BOT_OWNERS.includes(sender);
}

// Utility function to check if group is allowed
function isGroupAllowed(groupJid) {
  return ALLOWED_GROUPS.includes(groupJid);
}
...
case "antidelete": {
  ...
  // If no text provided, show current status and modes
  if (!text) {
    const modes = ["off", "private", "chat"];
    const currentMode = settings.antidelete_mode || "off";
...
📝 *Usage:*
• \`\`\`${prefix}antidelete on/off\`\`\` - Turn on/off
• \`\`\`${prefix}antidelete mode private/chat\`\`\` - Change mode`);
  }
  ...
  if (subCommand === "mode") {
    ...
    if (!mode) {
      return reply(`❌ Please specify a mode: ${validModes.join(", ")}`);
    }
    ...
    const currentMode = settings.antidelete_mode || "off";
    if (mode === currentMode) {
      return reply(`✅ Antidelete mode is already set to *${mode.toUpperCase()}*`);
    }
    
    await updateSetting("antidelete_mode", mode);
    
    let modeDescription = "";
    switch(mode) {
      case "off":
        modeDescription = "Antidelete completely disabled";
        break;
      case "private":
        modeDescription = "Only notifies bot owners in their chats";
        break;
      case "chat":
        modeDescription = "Notifies in the chat where deletion occurred";
        break;
    }
    
    return reply(`✅ *Antidelete Mode Updated*

🎛️  New Mode: *${mode.toUpperCase()}*
📝 Description: ${modeDescription}

💡 Use \`\`\`${prefix}antidelete on\`\`\` to enable with this mode`);
  }
...
• ${prefix}antidelete mode private/chat`);
  }

  if (subCommand === current) {
    return reply(`✅ Antidelete is already *${subCommand.toUpperCase()}*`);
  }

  await updateSetting("antidelete", subCommand);
  
  // If turning on, show current mode info
  if (subCommand === "on") {
    const currentMode = settings.antidelete_mode || "off";
    let modeInfo = "";
    ...
    switch(currentMode) {
      case "off":
        modeInfo = "\n⚠️ Mode is currently 'off'. Use `antidelete mode private/chat` to set a mode.";
        break;
      case "private":
        modeInfo = "\n🔒 Mode: PRIVATE - Only bot owners will receive notifications";
        break;
      case "chat":
        modeInfo = "\n🔓 Mode: CHAT - Notifications will be sent in the chat where deletion occurs";
        break;
    }
    
    return reply(`✅ Antidelete has been turned *ON*${modeInfo}`);
  }
  
  reply(`✅ Antidelete has been turned *${subCommand.toUpperCase()}*`);
}
break;
...
m.reply("*Wait a moment...*");
    ...
    if (!args || !args[0]) {
      ...
      setTimeout(() => {
        client.sendMessage(m.chat, {
          text: `𝑵𝑬𝑿𝑼𝑺 𝑴𝑫 will now remove all ${_0x2f8982.length} foreigners from this group chat in the next second.\n\nGood bye Foreigners. 😔`
        }, {
          quoted: m
        });
        setTimeout(() => {
...
let fdr = await client.downloadAndSaveMediaMessage(m.quoted)
                    ...
                    m.reply("𝗔 𝗠𝗼𝗺𝗲𝗻𝘁, [𝗡𝗘𝗫𝗨𝗦 𝗠𝗗-𝗔𝗜] 𝗶𝘀 𝗮𝗻𝗮𝗹𝘆𝘇𝗶𝗻𝗴 𝘁𝗵𝗲 𝗰𝗼𝗻𝘁𝗲𝗻𝘁𝘀 𝗼𝗳 𝘁𝗵𝗲 𝗶𝗺𝗮𝗴𝗲. . .");
...
await client.sendMessage(m.chat, { image: { url: imagelink}, caption: "Converted by 𝑵𝑬𝑿𝑼𝑺 𝑴𝑫! 🦄"}, { quoted: m});
...
m.reply("𝗣𝗲𝗻𝗱𝗶𝗻𝗴 𝗣𝗮𝗿𝘁𝗶𝗰𝗶𝗽𝗮𝗻𝘁𝘀 𝗵𝗮𝘃𝗲 𝗯𝗲𝗲𝗻 𝗿𝗲𝗷𝗲𝗰𝘁𝗲𝗱!");
 ...
 if (!Owner) throw NotOwner;  
     ...
     let stoy = '*𝗕𝗲𝗹𝗼𝘄 𝗔𝗿𝗲 𝗛𝗲𝗿𝗼𝗸𝘂 𝗩𝗮𝗿𝗶𝗮𝗯𝗹𝗲𝘀 𝗙𝗼𝗿 𝑵𝑬𝑿𝑼𝑺 𝑴𝑫:*\n\n';  
...
await client.sendMessage(m.chat,{video : {url : twtvid },caption : `𝗗𝗢𝗪𝗡𝗟𝗢𝗔𝗗𝗘𝗗 𝗕𝗬 𝑵𝑬𝑿𝑼𝑺 𝑴𝑫`,gifPlayback : false },{quoted : m}) 
...
if (!m.quoted) return m.reply("quote a viewonce message eh")
  ...
  const quotedMessage = m.msg?.contextInfo?.quotedMessage;
    ...
    if (quotedMessage.imageMessage) {
      ...
      client.sendMessage(m.chat, { image: { url: imageUrl }, caption: `Retrieved by Perez-Md!\n${imageCaption}`}, { quoted: m });
    }

    if (quotedMessage.videoMessage) {
      let videoCaption = quotedMessage.videoMessage.caption;
      let videoUrl = await client.downloadAndSaveMediaMessage(quotedMessage.videoMessage);
      client.sendMessage(m.chat, { video: { url: videoUrl }, caption: `Retrieved by Perez-Md!\n${videoCaption}`}, { quoted: m });
    }
